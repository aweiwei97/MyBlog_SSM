...
public class ContentServiceImpl implements IContentService {
    ...
    public PageInfo<ContentVo> getContents(Integer p, Integer limit) {
        ...
        example.setOrderByClause("created desc");//按“created”字段，排序
        example.createCriteria().andTypeEqualTo(Types.ARTICLE.getType()).andStatusEqualTo(Types.PUBLISH.getType());
        PageHelper.startPage(p, limit);
        List<ContentVo> data = contentDao.selectByExampleWithBLOBs(example);//检索的字段中包含大字段类型时，必须用selectByExampleWithBLOBs
    ...
    public ContentVo getContents(String id) {
        if (StringUtils.isNotBlank(id)) {   //判断某字符串是否非空，等于!isEmpty(String str)
            if (Tools.isNumber(id)) {       //判断是否可以为数字
    ...
    public void updateArticle(ContentVo contents) {
        ...
        int time = DateKit.getCurrentUnixTime();
        contents.setModified(time); //设置修改时间
        Integer cid = contents.getCid();
        contents.setContent(EmojiParser.parseToAliases(contents.getContent()));
