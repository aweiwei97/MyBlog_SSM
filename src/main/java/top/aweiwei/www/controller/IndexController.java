package top.aweiwei.www.controller;
...
import top.aweiwei.www.service.ICommentService;
import top.aweiwei.www.service.IContentService;
import top.aweiwei.www.service.IMetaService;
import top.aweiwei.www.service.ISiteService;
import top.aweiwei.www.utils.IPKit;
import top.aweiwei.www.utils.PatternKit;
...
public class IndexController extends BaseController {
    ...
    public String index(HttpServletRequest request, @PathVariable int p, @RequestParam(value = "limit", defaultValue = "6") int limit) {
        p = p < 0 || p > WebConst.MAX_PAGE ? 1 : p;   //当P小于0或者P大于最大页码时设置为p=1
       //获得已分页的内容
        PageInfo<ContentVo> articles = contentService.getContents(p, limit); //直接查找接口 ContentService的实现类的getContents（）方法
    ...
    public String getArticle(HttpServletRequest request, @PathVariable String cid) {
        ContentVo contents = contentService.getContents(cid);       //根据id或slug获取文章
        if (null == contents || "draft".equals(contents.getStatus())) {
            return this.render_404();   //正在起草中的
        }
    ...
    private void completeArticle(HttpServletRequest request, ContentVo contents) {
        if (contents.getAllowComment()) {    //判断是否允许评论
